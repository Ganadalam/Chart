...
export class CrosshairLayer extends Layer {
  ...
  init(ctx: CanvasRenderingContext2D, eventBus: any, chart?: any) {
    ...
    ctx.canvas.addEventListener("mouseleave", () => { this.mouse.x = -1; this.mouse.y = -1; });
    ctx.canvas.addEventListener("wheel", this.onZoom, { passive: false });
  }
  ...
  private onZoom = (e: WheelEvent) => {
    e.preventDefault();
    if (this.chart) this.chart.zoom(e.deltaY > 0 ? 0.9 : 1.1);
  };
...
//   draw() {
//     if (!this.ctx || this.mouse.x < 0) return;
//     const ctx = this.ctx;
//     ctx.save();
//     ctx.setLineDash([4, 4]);
//     ctx.strokeStyle = "#999";
...
//       ctx.fillStyle = "#fff";
//       if (d) ctx.fillText(String(d.close.toFixed(2)), ctx.canvas.width - 48, this.mouse.y + 6);

//       // Time
//       ctx.fillStyle = "rgba(0,0,0,0.7)";
//       ctx.fillRect(this.mouse.x - 40, ctx.canvas.height - 28, 80, 20);
//       ctx.fillStyle = "#fff";
//       if (d?.time) ctx.fillText(new Date(d.time).toLocaleTimeString(), this.mouse.x, ctx.canvas.height - 12);
//     }
