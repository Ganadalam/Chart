...
export default function App() {
  ...
  useEffect(() => {
    ...
    candleLayer.init(ctx, chart.getEventBus());
    smaLayer.init(ctx, chart.getEventBus());
    ...
    function renderLoop() {
      ...
      if (mouse.x >= 0) {
        const i = chart.xToIndex(mouse.x);
        const d = data[i];
        if (d) {
    ...
    const interval = setInterval(() => {
      ...
      data.push({ time: Date.now(), open, close, high, low });
...
// //     smaLayer.init(ctx, chart.getEventBus());
// //     axisLayer.init(ctx, chart.getEventBus());
// //     crosshair.setChart(chart);
// //     crosshair.init(ctx, chart.getEventBus());
