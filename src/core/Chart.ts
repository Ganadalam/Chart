export class Chart {
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    canvas.addEventListener("mousedown", (e) => {
      ...
      const move = (ev: MouseEvent) => {
        this.offset = startOffset + (ev.clientX - startX);
      };
      const up = () => {
  ...
  pushTick(price: number, time: number) {
    this.candles.push({ open: price, close: price, high: price, low: price, time });
    if (this.candles.length > 300) this.candles.shift();
    if (this.eventBus) this.eventBus.emit("chart:update", { candles: this.candles });
  }
  ...
  setWorkerRenderer(renderer: any) { this.renderer = renderer; }

  getEventBus() { return this.eventBus; }

  xToIndex(x: number) { return Math.floor((x - this.offset) / (8 * this.scale)); }
}
